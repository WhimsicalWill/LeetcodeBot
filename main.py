...
def run_discord_bot():
    ...
    async def report_score(ctx):
        ...
        try:
            ...
            problem = problems.get_problem_by_id(leetcode_api, problem_id)
            timestamp = datetime.datetime.now().date()
            data.add_user_problem(db_conn, message.author.id, int(problem_id), float(percentile), timestamp, problem.difficulty.level)
    ...
    async def progress(ctx):
        ...
        data.plot_user_progress(db_conn, leetcode_api, user_id)
